  ...
  try {
    ...
    switch (type) {
      case "vote":
        ...
        osc.start(); osc.stop(ctx.currentTime + 0.1); break;
      case "reveal":
        ...
        setTimeout(() => {
          ...
          o.frequency.value = 660; o.type = "triangle"; g.gain.value = 0.1;
          g.gain.exponentialRampToValueAtTime(0.01, ctx.currentTime + 0.3);
          o.start(); o.stop(ctx.currentTime + 0.3);
        }, 150); break;
      case "reset":
        osc.frequency.value = 330; osc.type = "square"; gain.gain.value = 0.06;
      ...
      case "consensus":
        ...
        setTimeout(() => {
          const o = ctx.createOscillator(); const g = ctx.createGain();
          o.connect(g); g.connect(ctx.destination);
          o.frequency.value = 784; o.type = "sine"; g.gain.value = 0.1;
      ...
      {Array.from({ length: 40 }, (_, i) => (
        <div key={i} className="absolute w-2 h-2 rounded-full animate-confetti"
          style={{
            ...
            backgroundColor: ["#22d3ee","#34d399","#fbbf24","#a78bfa","#f472b6"][i % 5],
...
export default function PokerRoom({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Load roomId and restore session from sessionStorage
  useEffect(() => {
    params.then((p) => {
      setRoomId(p.id);
      ...
      if (saved) {
        try {
          ...
          if (n && r) { setNickname(n); setRole(r); setJoined(true); }
        } catch { /* ignore */ }
      }
    });
  }, [params]);

  // Simple GET poll - no side effects on server
  ...
  // Sound effects on reveal transition only
  useEffect(() => {
    if (room.revealed && !prevRevealedRef.current && !soundFiredRef.current) {
      ...
      if (new Set(devVotes).size === 1 && devVotes.length > 1) {
        setTimeout(() => { playSound("consensus"); setShowConfetti(true); setTimeout(() => setShowConfetti(false), 3000); }, 500);
      }
    }
  ...
  // Start polling after join + re-join on server to ensure player exists
  useEffect(() => {
    if (!joined || !roomId || !nickname) return;
    // Re-join on mount/reconnect (idempotent on server)
  ...
  }, [joined, roomId, nickname, role, pollRoom]);
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    await sendAction({ action: "join", nickname: trimmed, role });
  ...
  if (!joined) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center px-4">
        <form onSubmit={handleJoin} className="w-full max-w-sm border border-cyan-400/30 bg-slate-900/80 backdrop-blur-sm rounded-xl p-8 space-y-6">
          <div className="text-center space-y-2">
            <h1 className="text-3xl font-bold text-cyan-400 font-mono tracking-tight">🃏 BytePoker</h1>
          ...
          <div className="flex flex-wrap justify-center gap-5">
            {room.players.map((p) => (
              <div key={p.nickname} className="flex flex-col items-center gap-2">
                <div className={`relative w-16 h-24 transition-all duration-500 ${flipCards && p.role === "dev" ? "animate-flip" : ""}`} style={{ perspective: "600px" }}>
        ...
        {role === "host" && (
          <section className="space-y-4">
            ...
            <div className="flex justify-center gap-4">
              <button onClick={() => sendAction({ action: "reveal" })} disabled={room.revealed} className={`px-6 py-3 border font-semibold rounded-lg transition-all text-sm ${room.revealed ? "border-slate-700 bg-slate-800 text-slate-500 cursor-not-allowed" : "bg-amber-500/10 border-amber-500/50 text-amber-500 hover:bg-amber-500/20 hover:scale-105 active:scale-95"}`}>👁 Revelar Cartas</button>
              <button onClick={() => sendAction({ action: "reset" })} className="px-6 py-3 bg-slate-800 border border-slate-700 text-slate-300 font-semibold rounded-lg hover:bg-slate-700 hover:scale-105 active:scale-95 transition-all text-sm">🔄 Nova Rodada</button>
            </div>
          </section>
        )}
